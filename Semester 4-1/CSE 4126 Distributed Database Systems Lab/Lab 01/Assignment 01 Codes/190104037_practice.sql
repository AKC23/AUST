...
select A_Name from Author where 
AuthorID = (select AuthorID from 
(Select AuthorID, count(AuthorID) From Book Group by AuthorID 
Order by count(AuthorID) desc) where rownum = 1);
...
join Book on Borrows.Borrows_BookID = Book.BookID and Book.B_Genre = 'Non-Fiction';

/*2c. Show the dates on which “Macbeth” was borrowed. */
select Borrows.Borrows_DateBorrowed from Borrows 
left join Book on Borrows.Borrows_BookID = Book.bookID 
